...
public:
    Traceur(int w, int h, const char *name, double _f(double), double _xmin, double _xmax, double _ymin, double _ymax, unsigned short _pixel_step)
        : EZWindow(w, h, name),
          ...
          curi(-1), curj(-1) { setDoubleBuffer(true); }// On active le double buffer pour limiter le scintillement de l'affichage.
...
//= La fonction poly ==========================================================
double poly(double x) { return -x * x * x + 4 * x * x - 5 * x + 2; }
